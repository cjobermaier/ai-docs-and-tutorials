...
<Note>
  Grounded Docs requires **Node.js 22+**. Run `node --version` to confirm before starting.
</Note>
...
To improve the accuracy of AI reviews, you provide versioned documentation to your AI.

## Set up the tutorial file

<Steps>
  <Step title="Create a working directory">
    In your terminal, create a directory for this tutorial and download the React 18 `forwardRef` tutorial file:
    ...
    <Tabs>
      ...
      <Tab title="Windows PowerShell">
        ```powershell
        New-Item -ItemType Directory -Path versioned-docs-tutorial
        Set-Location versioned-docs-tutorial
        Invoke-WebRequest -OutFile react-18-forwardref-tutorial.md https://raw.githubusercontent.com/cjobermaier/public-resources/main/increase-content-accuracy/react-18-forwardref-tutorial.md
        ```
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Launch Claude Code from this directory">
    ...
    Later commands reference `react-18-forwardref-tutorial.md` as a relative path, so Claude must be running from this directory. If you already have a Claude Code session open elsewhere, exit it and start a fresh one here.
  </Step>
</Steps>

## Indexing versioned documentation with Grounded Docs

In Claude Code, `/docs-manage <prompt>` invokes the docs-manage skill — Claude reads your natural-language instructions and runs the right underlying commands. The same pattern applies to `/docs-search` later in this tutorial.
...
<Steps>
  <Step title="Index both React versions">
    ...
    Claude crawls both sites and stores the content locally. Allow each permission prompt as it runs. If you deny one by mistake, re-run the `/docs-manage` skill.
...
<Steps>
  <Step title="Add the citation block format">
    By default, the `docs-search` skill returns generic AI output. You enhance it so
    Claude produces structured, citation-backed findings.

    Claude updates the `docs-search` skill to include:
    ...
    <Tip>
      Review the diff before accepting. Claude should only be adding the new fact-checking workflow and citation format — not rewriting the rest of the SKILL.md.
    </Tip>
  </Step>
  ...
  <Step title="Verify the enhancement">
    ...
    Claude should describe the four-field format (Doc path, Public URL, Quote, Finding). If it doesn't, re-run the prompt in the previous step.
  </Step>
</Steps>

## Fact-check the tutorial

Fact-checking is the process of scanning a file for claims and verifying each one against indexed documentation. Claude searches the indexed docs for supporting or contradicting text and reports each finding with a verbatim quote and source URL. Your exact quotes and verdicts may differ between runs — focus on the structure and the overall conclusions, not exact wording.
...
<Steps>
  ...
  All six claims pass against React 19.2.0.
  </Step>
</Steps>
